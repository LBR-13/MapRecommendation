...
int PIRServer::serialize_reply(PirReply& reply, stringstream& stream) {
  ...
  for (int i = 0; i < reply.size(); i++) {
    ...
    // cout<< reply[i].save(stream) << endl; // 46301 46415 46357 46350 differs every time
  }
  return output_size;
}
...
void PIRServer::set_galois_keys_from_stream(uint32_t client_id, stringstream& galois_keys_stream) {
  GaloisKeys galois_keys;
  galois_keys.load(*context_, galois_keys_stream);
  this->set_galois_key(client_id, galois_keys);
}
